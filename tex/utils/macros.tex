...
\newcommand{\neventplotannotations}{\jroedit{}{The estimates are based on the
    number of events with the maximum a posteriori probability (MAP).}
    For each plot,
...
\newcommand{\modelplotannotations}{Each model is represented along the plot
    ...
    The estimates are based on the model with the maximum a posteriori
    probability (MAP).
    For each plot,
    the proportion of \datasets for which the model with the largest
    posterior probability matched the true model---$p(\hat{\divtimesets}
    = \divtimesets)$---is shown in the upper left corner,
    the median posterior probability of the correct model across all
    \datasets---$\widetilde{p(\divtimesets|\alldata)}$---is shown in the upper
    right corner, and
    the proportion of \datasets for which the true model was
    included in the 95\% credible set---$p(\divtimesets \in
    \textrm{CS})$---is shown in the lower right.
}
...
\newcommand{\modelshadingdescription}{The number of \datasets that fall
    within each possible cell of true versus estimated model is
    shown, and cells with more \datasets are shaded darker.
}
