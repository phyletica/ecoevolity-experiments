...
\newcommand{\validationfiguregriddescription}{The first four columns represent
    different distributions on the relative effective size of the ancestral
    population, decreasing in variance from left to right.
    The fifth column represents datasets simulated (and analyzed) with the
    effective size ($\epopsize{}\murate{}$) of all populations are fixed to
    0.002.
    The first two rows represent data sets simulated with 500,000 characters
    for each pair of populations; the first row shows the results of analyses
    of all the characters, and the second shows the results of only analyzing
    the variable characters.
    The last two rows are the same as the first two, except with data sets
    simulated with 100,000 characters for each pair of populations.
}
