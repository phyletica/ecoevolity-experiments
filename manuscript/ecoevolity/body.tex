...
advantage of more information in those loci, in order to more reliably estimate
the number of divergence events and the assigngment of taxa to those events.
...
Let's use \divtimemodel to represent the divergence model, which comprises
...
in units of time determined by $\murate$ (e.g., expected subsitutions per site
if $\murate = 1$).
The motivation of our move is to keep the expected gene tree root height
...
\end{equation}
This will allow us to jointly and efficiently explore the space of \divtime
and \sepopsize[\rootpopindex] when there is little information in the data to
tease them apart.
...
root population
\[
    \sepopsize[\rootpopindex]{}\proposed = \sepopsize[\rootpopindex]e^{\uniformdeviate}.
\]
Now, we use the relationship in Equation~\ref{eq:timerootsizeequality} to
determine the corrsponding proposed value of population divergence time,
\begin{equation}
    \divtime\proposed =
    \divtime + 2\sepopsize[\rootpopindex] - 2\sepopsize[\rootpopindex]{}\proposed.
\end{equation}
...
\begin{equation}
    ...
    \frac{g(\uniformdeviate\proposed)}{g\proposed(\uniformdeviate)} |\det(J)|,
    \label{eq:greenhastings}
\end{equation}
which is the ratio of the probabilities of drawing the random
deviates multiplied by the absolute value of the determinant
of a matrix of Jacobian terms.
Because the forward and reverse random deviates are uniform,
$\frac{g(\uniformdeviate\proposed)}{g\proposed(\uniformdeviate)} = 1$,
and the Hastings ratio reduces to just the Jacobian term,
\begin{eqnarray}
    J & = & \left|\begin{array}{ccc}
        \frac{\partial \sepopsize[\rootpopindex]{}\proposed}{\partial \sepopsize[\rootpopindex]} &
        \frac{\partial \sepopsize[\rootpopindex]{}\proposed}{\partial \divtime} &
        \frac{\partial \sepopsize[\rootpopindex]{}\proposed}{\partial \uniformdeviate} \\
        \frac{\partial \divtime\proposed}{\partial \sepopsize[\rootpopindex]} &
        \frac{\partial \divtime\proposed}{\partial \divtime} &
        \frac{\partial \divtime\proposed}{\partial \uniformdeviate} \\
        \frac{\partial \uniformdeviate\proposed}{\partial \sepopsize[\rootpopindex]} &
    ...
    \end{array}\right| \\
    & = & -e^{\uniformdeviate} \\
    |\det(J)| & = & |-e^{\uniformdeviate}| = e^{\uniformdeviate}.
\end{eqnarray}
This give us $e^{\uniformdeviate}$ for the hasting ratio of this proposal.
...
of divergence times.
However, we can easily extend this move to change the size of the ancestral
population size of pairs $j, k, \ldots n$ that share their divergence time with
pair $i$.
...
acceptance rate.
We use this random deviate to propse a new value for the divergence time,
\[
...
that have been scaled, denoted as $n$,
...
For the data sets simulated with 100,000 and 500,000 characters,
...
the number of sampled characters (100k to 500k) is relatively modest
(Figures \ref{fig:valdivtimes}, S\ref{fig:valleafsizes}, \&
S\ref{fig:valrootsizes}),
...
The accuracy of divergence time estimates does not appear to be affected by the
...
trees.
Importantly, this effect of underestimating posterior uncertainty is
...
Clearly, variable site patterns are being lost during the acquistion and
assembly of the RADseq data, and the model is senstive to these missing
...
When there is no model mispecification, our new approach is accurate in
estimating divergence times, effective population sizes, and divergence models
across all the conditions we simulated, including missing data
(Figures
\labelcref{fig:valdivtimes,fig:valpostprobs,fig:valnevents}
and
S\labelcref{fig:valleafsizes,fig:valrootsizes}).
We ee increased precision when we increase the number of sampled characters
from 100,000 to 500,000, but the increase is modest relative to the five-fold
increase in data,
especially when compared to the increases in precision associated with keeping
constant characters and increasing prior information about the relative
effective size of the root population.

\subsection{To exclude linked sites or not to exclude?}
The increased precision associated with retaining the constant characters
creates an interesting question when analyzing DNA sequence data from
...
Third, the results from the \emph{Gekko} RADseq data are nonsensical when
only variable characters are analyzed.
...
The accuracy of divergence time estimates is not affected
...
is correct 
(Figure~\ref{fig:valpostprobs}).
The accuracy in estimating parameter values and posterior probabilities is true
even when constant characters are excluded, demonstrating that our likelihood
correction for those missing constant characters is working correctly
(Equation~\ref{eq:variablecomparisonlikelihood}).
...
This bias is smaller when the constant patterns are retained in
the dataset
(Figure
\ref{fig:filtereddivtimes},
S\ref{fig:filteredleafsizes}
...
constant characters were excluded.
Clearly, variable site patterns are being lost during the acquistion and
assembly of the RADseq data, and the model is extremely senstive to these
missing variable sites when only variable characters are analyzed.
